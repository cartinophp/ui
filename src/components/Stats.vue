<template>
  <div :class="ui.root">
    <!-- Icon -->
    <div v-if="icon" :class="ui.iconContainer">
      <Icon :name="icon" :class="ui.icon" />
    </div>
    ...
    <div :class="ui.content">
      ...
      <!-- Value -->
      <div :class="ui.value">
        <span v-if="prefix" :class="ui.prefix">{{ prefix }}</span>
        <span>{{ formattedValue }}</span>
        <span v-if="suffix" :class="ui.suffix">{{ suffix }}</span>
      </div>
      ...
      <!-- Change Indicator -->
      <div v-if="change !== undefined" :class="ui.change">
        <Icon 
          v-if="changeIcon" 
          :name="changeIcon" 
          :class="ui.changeIcon" 
      ...
      <!-- Progress -->
      <div v-if="progress !== undefined" :class="ui.progressContainer">
        ...
        <span v-if="showProgress" :class="ui.progressText">
    ...
    <div v-if="trend?.length" :class="ui.trend">
...
const progressPercentage = computed(() => {
  ...
  return Math.min(Math.max(props.progress, 0), props.progressMax!) / props.progressMax! * 100
})
...
const trendPath = computed(() => {
  ...
  const pathPoints = points.map((value, index) => {
    ...
    const y = trendHeight.value - 2 - ((value - minY) / range) * (trendHeight.value - 4)
    return `${x},${y}`
  })
